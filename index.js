class Chevalier {
    ...
    attack(target){
        ...
        if(target.strength - this.strength > 0){
            degats = (target.strength - this.strength) / 2;
        } else {
    ...
    usePotion(){
        ...
        return `${this.name} utilise une potion, PV actuels : ${this.life.toFixed(2)}, potions restantes : ${this.potions}`;
    }
...
document.getElementById('knights').addEventListener('submit', function(e) {
    ...
    const magic1 = parseInt(document.getElementById('magic1').value);

    const name2 = document.getElementById('nameInput2').value;
    const strength2 = parseInt(document.getElementById('strength2').value);
    const magic2 = parseInt(document.getElementById('magic2').value);
